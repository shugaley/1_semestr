...
size_t Create_Func(tree_elem* pos_func)
{
    ...
    func[i_func].name = strdup(pos_func->data.word);
    func[i_func].n_param = Count_FuncNParam(pos_func);
    func[i_func].var = (var_table*)calloc(MAXSIZE_VAR_TABLE, sizeof(var_table));
...
void Print_DumpVar(tree_elem* pos)
{
    ...
    fprintf(stderr, "i \t n_param \t\t\t\t var \t\t\t\t\t name\n");
